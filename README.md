...
2. 安装到备用机。**首次使用必须完成 4 件事**：
   1. 打开 App → 填「打卡时间 / 目标考勤应用 / 关键词 / 邮箱信息」→ 点「保存并调度」。
      - **打卡时间**：最多勾选 5 个时间点（如上班 08:55、下班 18:05），勾选并填好时分的即当天要执行的次数；**开始日期**：可点「选择日期」设定从哪一天起生效（留空 = 保存后立即生效）。之后**每天自动顺延**：当天所有打卡点执行完会自动排到下一日，无需再手动操作；改动时间点/开始日期后重新保存即按新设置顺延。
      - 目标应用**推荐直接点「选择应用」按钮**，从已安装应用列表（可搜索名称或包名）点选，避免手输名称匹配失败；手输名称后若识别不到，会自动弹出相似应用列表供你挑选；
   2. 点「无障碍设置」开启「自动打卡助手」；
